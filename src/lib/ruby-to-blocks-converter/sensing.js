...
const DragMode = ["draggable", "not draggable"];
        ...
        if ((this._isSelf(receiver) || receiver === Opal.nil) && !rubyBlock) {
            switch (name) {
                ...
                case "color_is_touching_color?":
                    if (
                        args.length === 2 &&
                        this._isColorOrBlock(args[0]) &&
                        this._isColorOrBlock(args[1])
                    ) {
                        block = this._createBlock(
                ...
                case "drag_mode=":
                    if (
                        args.length === 1 &&
                        ((this._isString(args[0]) &&
                            DragMode.indexOf(args[0].toString()) >= 0) ||
                            (args[0] &&
                                (args[0].type === "true" ||
                                    args[0].type === "false")))
                    ) {
                ...
                case "sprite":
                    if (args.length === 1 && this._isString(args[0])) {
                        block = this._createRubyExpressionBlock(
                            spriteCall(args[0]),
                            node
                        );
                    }
                ...
                case "::Keyboard":
                    if (
                        ...
                        (this._isBlock(args[0]) ||
                            (this._isString(args[0]) &&
                                KeyOptions.indexOf(args[0].toString()) >= 0))
                    ) {
                        block = this._createBlock(
                            "sensing_keypressed",
                            "value_boolean"
            ...
            args.length === 0 &&
            this._equalRubyExpression(receiver, TimeNow)
        ) {
            let currentMenu;
            switch (name) {
                case "year":
            ...
            if (args.length === 0) {
                ...
                if (property) {
                    const spriteName = getSpriteName(
                        this._getRubyExpression(receiver)
                    );

                    block = this._changeBlock(receiver, "sensing_of", "value");
                    delete this._context.blocks[
                        receiver.inputs.EXPRESSION.block
                    ];
                    delete receiver.inputs.EXPRESSION;
