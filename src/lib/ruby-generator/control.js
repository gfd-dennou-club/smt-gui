...
export default function (Generator) {
    Generator.control_wait = function (block) {
        const secs =
            Generator.valueToCode(block, "DURATION", Generator.ORDER_NONE) || 0;
        return `sleep(${secs})\n`;
    };
    ...
    Generator.control_if = function (block) {
        const operator =
            Generator.valueToCode(block, "CONDITION", Generator.ORDER_NONE) ||
            false;
        const branch = Generator.statementToCode(block, "SUBSTACK") || "";
        return `if ${operator}\n${branch}end\n`;
    };
    ...
    Generator.control_stop = function (block) {
        const target = Generator.quote_(
            Generator.getFieldValue(block, "STOP_OPTION") || "all"
        );
        return `stop(${target})\n`;
    };
    ...
    Generator.control_create_clone_of_menu = function (block) {
        const target = Generator.quote_(
            Generator.getFieldValue(block, "CLONE_OPTION") || "_myself_"
        );
