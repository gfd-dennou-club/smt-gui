...
const SensingAttributeMenu = ["motion", "direction"];

const SensingSubjectMenu = ["Stage", "this sprite"];
const SensingSubjectMenuLower = SensingSubjectMenu.map((x) => x.toLowerCase());

const VideoStateMenu = ["off", "on", "on-flipped"];
...
const VideoConverter = {
    register: function (converter) {
        ...
        converter.registerCallMethodWithBlock(
            VideoSensing,
            "when_video_motion_greater_than",
            1,
            0,
            (params) => {
        ...
        converter.registerCallMethod(VideoSensing, "video_on", 2, (params) => {
            ...
            if (converter.isString(args[0])) {
                const index = SensingAttributeMenu.indexOf(
        ...
        converter.registerCallMethod(
            ...
            (params) => {
                ...
                if (!converter.isStringOrBlock(args[0])) return null;
                if (converter.isString(args[0])) {
                    const index = VideoStateMenu.indexOf(
                        args[0].toString().toLowerCase()
                    );
                    if (index < 0) return null;
