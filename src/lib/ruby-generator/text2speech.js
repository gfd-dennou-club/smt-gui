...
export default function (Generator) {
    Generator.text2speech_speakAndWait = function (block) {
        const words =
            Generator.valueToCode(block, "WORDS", Generator.ORDER_NONE) || null;
        return `text2speech.speak(${words})\n`;
    };

    Generator.text2speech_setVoice = function (block) {
        const voice = Generator.valueToCode(
            block,
            "VOICE",
    ...
    Generator.text2speech_menu_voices = function (block) {
        const voice = Generator.quote_(
            Generator.getFieldValue(block, "voices") || "ALTO"
        );
        return [voice, Generator.ORDER_ATOMIC];
    };

    Generator.text2speech_setLanguage = function (block) {
        const language = Generator.valueToCode(
            block,
            "LANGUAGE",
            Generator.ORDER_NONE
        );
    ...
    Generator.text2speech_menu_languages = function (block) {
        const language = Generator.quote_(
            Generator.getFieldValue(block, "languages") || "ja"
        );
