...
const createBlockWithMessage = function (opcode, message, defaultMessage) {
    ...
    this._addTextInput(
        block,
        "MESSAGE",
        this._isNumber(message) ? message.toString() : message,
        defaultMessage
    );
    return block;
};
...
const ForwardBackward = ["forward", "backward"];
            ...
            switch (name) {
                ...
                case "think":
                    if (
                        ...
                        block = createBlockWithMessage.call(
                            this,
                            opcode,
                            args[0],
                            defaultMessage
                ...
                case "set_effect":
                    if (
                        ...
                        Effects.indexOf(args[0].toString().toUpperCase()) >=
                            0 &&
                        this._isNumberOrBlock(args[1])
                    ) {
                        let opcode;
                ...
                case "go_to_layer":
                    if (
                        args.length === 1 &&
                        this._isString(args[0]) &&
                        FrontBack.indexOf(args[0].toString()) >= 0
                    ) {
                        block = this._createBlock(
                            "looks_gotofrontback",
                            "statement"
                ...
                case "go_layers":
                    if (
                        ...
                        ForwardBackward.indexOf(args[1].toString()) >= 0
                    ) {
                        block = this._createBlock(
