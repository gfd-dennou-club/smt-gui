    ...
    onSend: function (receiver, name, args, rubyBlockArgs, rubyBlock) {
        ...
        if (!this._isVariableBlock(receiver)) {
            switch (name) {
                case "[]":
                    if (
                        this._isStringOrBlock(receiver) &&
                        args.length === 1 &&
        ...
        switch (name) {
            ...
            case "%":
                if (rh) {
                    if (
                        this._isNumberOrBlock(receiver) &&
                        ...
                        name === "+" &&
                        (this._isStringOrBlock(receiver) ||
                            this._isStringOrBlock(rh))
                    ) {
                        block = this._createBlock("operator_join", "value");
                        this._addTextInput(
                            block,
                            "STRING1",
                            this._isNumber(receiver)
                                ? receiver.toString()
                                : receiver,
                            "apple"
                        );
                        this._addTextInput(
                            block,
                            "STRING2",
                            this._isNumber(rh) ? rh.toString() : rh,
                            "banana"
                        );
                        return block;
                    }
            ...
            case "!":
                if (
                    args.length === 0 &&
                    this._isFalseOrBooleanBlock(receiver)
                ) {
            ...
            case "log10":
                if (
                    rh &&
                    this._isConst(receiver) &&
                    receiver.toString() === "::Math" &&
                    this._isNumberOrBlock(rh)
                ) {
                    let operator;
                    switch (name) {
    ...
    onAnd: function (operands) {
        ...
        if (!this._isFalse(operands[0])) {
            this._addInput(
                block,
                "OPERAND1",
                this._createTextBlock(operands[0])
            );
        }
        if (!this._isFalse(operands[1])) {
            this._addInput(
                block,
                "OPERAND2",
                this._createTextBlock(operands[1])
            );
        }
        return block;
    },
