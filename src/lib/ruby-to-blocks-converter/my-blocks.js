...
const MyBlocksConverter = {
        ...
        if (this._isSelf(receiver) || receiver === Opal.nil) {
            ...
            if (procedure) {
                if (procedure.argumentIds.length === args.length) {
                    ...
                    if (
                        this._context.procedureCallBlocks.hasOwnProperty(
                            procedure.id
                        )
                    ) {
                        this._context.procedureCallBlocks[procedure.id].push(
                            block.id
                        );
                    } else {
                    ...
                    args.forEach((arg, i) => {
                        ...
                        if (this._isFalseOrBooleanBlock(arg)) {
                            if (
                                procedure.argumentVariables[i].isBoolean ||
                                this._changeToBooleanArgument(
                                    procedure.argumentNames[i]
                                )
                            ) {
                        ...
                        if (
                            !procedure.argumentVariables[i].isBoolean &&
                            (this._isNumberOrBlock(arg) ||
                                this._isStringOrBlock(arg))
                        ) {
                            this._addTextInput(
                                block,
                                argumentId,
                                this._isNumber(arg) ? arg.toString() : arg,
                                ""
                            );
                            return;
                        }
                        throw new RubyToBlocksConverterError(
                            ...
                            `invalid type of My Block "${name}" argument #${
                                i + 1
                            }`
                        );
                    });
    ...
    onDefs: function (node, saved) {
        ...
        const customBlock = this._createBlock(
        ...
        this._process(node.children[2]).forEach((n) => {
            ...
            const inputBlock = this._createBlock(
        ...
        procedure.argumentVariables.forEach((v, i) => {
            if (v.isBoolean) {
                ...
                procedure.argumentBlocks[i].opcode =
                    "argument_reporter_boolean";
                this._setBlockType(
                    procedure.argumentBlocks[i],
                    "value_boolean"
                );
            }
        });
