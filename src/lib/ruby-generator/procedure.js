...
export default function (Generator) {
    Generator.procedures_definition = function (block) {
        ...
        const customBlock = Generator.getInputTargetBlock(
            block,
            "custom_block"
        );
    ...
    const blockToMethod = function (block, isCall) {
        let methodName = block.mutation.proccode
            .split(" ")
            .filter((i) => !/^%[sb]$/.test(i))
            .join("_");
        ...
        const paramNamesIdsAndDefaults =
            Generator.currentTarget.blocks.getProcedureParamNamesIdsAndDefaults(
        ...
        if (isCall) {
            ...
            for (let i = 0; i < paramNamesIdsAndDefaults[0].length; i++) {
                ...
                let paramName = Generator.escapeVariableName(
                    paramNamesIdsAndDefaults[0][i]
                );
                paramName = paramName.replace(/^[A-Z]/, (firstLetter) =>
    ...
    Generator.argument_reporter_boolean = function (block) {
        return [
            Generator.escapeVariableName(
                Generator.getFieldValue(block, "VALUE")
            ),
